...
public class BobController extends HttpServlet {
	...
	IOException {
		...
		if(url.indexOf("list.do") != -1) {
			...
			List<BobDTO> list=dao.list();
			request.setAttribute("list", list);
			String page="/bob/list.jsp";
			...
			dao.insert(dto);
			String page="/bob_servlet/list.do";
			...
			//번호에 해당하는 게시물 리턴
			BobDTO dto=dao.viewReplace(num);
			//request 영역에 저장
			request.setAttribute("dto", dto);
			//화면 전환
			String page="/bob/view.jsp";
			...
			//댓글 목록 리턴
			List<BobCommentDTO> list=dao.commentList(num);
			//request 영역에 저장
			request.setAttribute("list", list);
			//출력 페이지로 이동
			String page="/bob/comment_list.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
		}else if(url.indexOf("comment_add.do") != -1) {
			BobCommentDTO dto=new BobCommentDTO();
			//게시물 번호
			int bob_num=Integer.parseInt(
					request.getParameter("bob_num"));
			String writer=request.getParameter("writer");
			String content=request.getParameter("content");
			dto.setBob_num(bob_num);
			dto.setWriter(writer);
			dto.setContent(content);
			dao.commentAdd(dto);
			...
			int num=Integer.parseInt(request.getParameter("num"));
			BobDTO dto=dao.view(num);
			dto.setContent("===게시물의 내용===\n"+dto.getContent());
			request.setAttribute("dto", dto);
			String page="/bob/reply.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			if(result != null){ //비밀번호가 맞으면
				...
				page=contextPath+"/bob_servlet/view.do?num="
			+num+"&message=error";
